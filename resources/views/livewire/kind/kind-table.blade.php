                ...
                <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="overflow-hidden">
                        <table class="min-w-full overflow-hidden rounded-md">
                            ...
                            <tr>
                                <th class="text-sm font-medium text-gray-900 px-6 py-4 text-left">Img</th>
                            ...
                            @foreach($kinds as $kind)
                                <tr class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                                    ...
                                    <td class="px-6 mt-2">
                                        @if (!empty($kind->img))
                                            <img src="{{asset('storage/img/'.$kind->img)}}"
                                                 style="height: 25px;width: auto">
                                        @else
                                            No Img
                                        @endif
                                    </td>
                                    <td class="px-6 mt-2 ">{{$kind->name}}</td>
                                    <td class="px-6 mt-2 ">{{$kind->description}}</td>
                                    <td class="flex justify-end mx-4 my-2">
                                        <!-- Inside existing Livewire component -->
                                        @can('kind-edit')
                                            <button
                                                wire:click='$emit("openModal", "kind.kind-edit", {{json_encode(["kind" => $kind->id])}})'
                                        ...
                                        @can('kind-delete')
                                            <button
                                                wire:click='$emit("openModal", "kind.kind-delete", {{json_encode(["kind" => $kind->id])}})'
